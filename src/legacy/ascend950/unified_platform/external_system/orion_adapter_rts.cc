...
{
    // host-only 模式直接返回 0，避免抛异常打断上层调用链。
    if (hccl::HostModeDetector::IsHostOnly()) {
        HCCL_INFO("[HrtGetDeviceCount] host-only mode fallback count[0].");
        return 0;
    }
    u32       count = 0;
...
void *HrtMalloc(u64 size, aclrtMemType_t memType)
{
    ...
    if (ret == ACL_ERROR_RT_MEMORY_ALLOCATION) {
        RPT_INPUT_ERR(true, "EI0007", std::vector<std::string>({"resource_type", "resource_info"}),
                            std::vector<std::string>({"DeviceMemory", std::string("aclrtMallocWithCfg, size:") + std::to_string(size) + " bytes"}));
...
void *HrtMallocHost(u64 size)
{
    ...
    if (ret != ACL_SUCCESS) {
        RPT_INPUT_ERR(true, "EI0007", std::vector<std::string>({"resource_type", "resource_info"}),
                            std::vector<std::string>({"HostMemory", std::string("aclrtMallocHostWithCfg, size:") + std::to_string(size)+ " bytes"}));
...
HcclResult HrtGetP2PStatus(u32 deviceLogicId, u32 devicePhyId, uint32_t *status)
{
    ...
    HCCL_DEBUG("rt getp2pstatus deviceLogicId[%u] and devicePhyId[%u] ret[%d], status[%u]",
