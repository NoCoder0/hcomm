...
static HcclResult IsPcieStdMainboard(uint32_t devLogicId, bool &isPcieStd)
{
    ...
    if (IsHostNicPluginForceLoadEnabled() &&
        ...
        HCCL_RUN_WARNING("[TpManager][%s] skip device mainboard lookup for Host-only TP manager resource id[%u], "
...
int32_t ResolveHostUbTpManagerResourceId()
{
    ...
    HCCL_RUN_WARNING("[TpManager][%s] use dedicated Host-only TP manager resource id[%d], env[%s]=1, test only.",
...
void TpManager::Init()
{
    ...
    if (IsHostNicPluginForceLoadEnabled() &&
        ...
        HCCL_RUN_WARNING("[TpManager][%s] force Host-only phyId[%u] for resource id[%u], env[%s]=1, test only.",
