...
HcclResult hrtMalloc(void **devPtr, u64 size, bool level2Address)
{
    ...
    RPT_ENV_ERR((ret != ACL_SUCCESS), "EI0007", std::vector<std::string>({"resource_type", "resource_info"}), \
        std::vector<std::string>({"DeviceMemory", std::string("hrtMalloc, size:") + std::to_string(size)+ " bytes"}));
...
#endif
    RPT_ENV_ERR((ret != ACL_SUCCESS), "EI0007", std::vector<std::string>({"resource_type", "resource_info"}), \
        std::vector<std::string>({"HostMemory", std::string("hrtMallocHost, size:") + std::to_string(size)+ " bytes"}));
